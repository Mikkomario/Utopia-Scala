...
object DrawableHandler2
{
	/**
	  ...
	  */
	def apply(elements: IterableOnce[handling.Drawable2] = Vector(), drawDepth: Int = DepthRange.default,
	          customizer: Option[Drawer3 => Drawer3] = None) = new DrawableHandler2(elements, drawDepth, customizer)
	
	/**
	  * @param element A drawable element
	  * @return a new handler with a single drawable element
	  */
	def apply(element: handling.Drawable2): DrawableHandler2 = apply(Vector(element))
	
	/**
	  * @return A new handler with all specified drawables
	  */
	def apply(first: handling.Drawable2, second: handling.Drawable2, more: handling.Drawable2*): DrawableHandler2 =
...
/**
  ...
  */
class DrawableHandler2(initialElements: IterableOnce[handling.Drawable2], override val drawDepth: Int,
                       customizer: Option[Drawer3 => Drawer3])
	extends DeepHandler[handling.Drawable2](initialElements) with handling.DrawableHandler2
{
	/**
