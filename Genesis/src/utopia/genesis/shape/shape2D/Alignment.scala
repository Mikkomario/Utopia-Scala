...
import utopia.genesis.shape.shape1D.LinearAlignment
import utopia.genesis.shape.shape1D.LinearAlignment.{Close, Far, Middle}
...
sealed trait Alignment extends TwoDimensional[LinearAlignment]
{
	...
	/**
	  ...
	  */
	def horizontalDirection: Option[HorizontalDirection] = horizontal.direction.map { HorizontalDirection(_) }
	/**
	  * @return The direction this alignment will move the items vertically. None if this alignment doesn't
	  *         specify a direction (centered)
	  */
	def verticalDirection: Option[VerticalDirection] = vertical.direction.map { VerticalDirection(_) }
