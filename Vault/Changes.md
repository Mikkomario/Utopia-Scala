...
- **ConditionElement**
  - Added `.within(MayHaveOrderedEnds)` and `.notWithin(...)`
  - Added `.notBetween(...)`
- **DeprecatesAfter**
  - Added `.deprecatedSinceCondition(Instant)`
- **IdOrInserted** (object)
  - Added a new implicit constructor from an existing ID (Int)
  - Added a new constructor that also works for types other than **HasId**
- **IntIndexFilterableView**
  - Added `.withId(ConditionElement)`
- **SqlSegment** (object)
  - Added new `.combine(...)` and `.concat(...)` functions
- **Store**
  - Object
...
### Other
- **Table**'s `.toJoinsFrom(...)` now prioritizes the join route based on the type of referencing columns, 
  reference direction and the number of joining tables
- **JoiningDbReader** now uses left joins where the joined (right) reader utilizes them
- **JoinType** now extends **SelfComparable**
- **IdOrInserted** and **StoreResult** no longer require the generic type parameter (A) to be of type **HasId**
- **ConditionElement**'s `.isBetween(HasTwoEnds)` no longer requires the ends to be of type **ConditionElement**
- Improved error messages generated by **ColumnLengthRule**
- Optimized **IntSet**-building in **AccessColumnValues**
- Further optimized `Condition.indexIn(...)`
- Minor optimization in **TargetingManyRowsLike**'s `.toMapBy(...)` implementation
- Internal refactoring within SQL segment combine-logic
