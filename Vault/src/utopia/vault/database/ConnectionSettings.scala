 ...
 * @param charsetName Name of the charset used when reading from DB. Please refer to MySql character set names in
...
{
	/**
	 * @return String inserted to driver manager connection in order to specify charset
	 */
	def charsetString = if (charsetName.isEmpty) "" else s"?useUnicode=true&characterEncoding=$charsetName"
}