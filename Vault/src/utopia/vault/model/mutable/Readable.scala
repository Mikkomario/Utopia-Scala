...
trait Readable extends Storable
{
     ...
     */
	def set(data: Model[Property]): Unit
	 ...
	 * has an index
	 */
	def setAndUpdate(data: Model[Property])(implicit connection: Connection) =
	{
	    set(data)
	 ...
	 */
	def setAndPush(data: Model[Property], writeNulls: Boolean = false)(implicit connection: Connection) =
	{
	    set(data)
