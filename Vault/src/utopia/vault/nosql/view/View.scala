...
trait View
{
	...
	/**
	  ...
	  *         None if no condition should be applied.
	  */
	def accessCondition: Option[Condition]
	...
	/**
	  ...
	  */
	def nonEmpty(implicit connection: Connection) = accessCondition match {
	...
	/**
	  ...
	  */
	def mergeCondition(additional: Condition) = accessCondition.map { _ && additional }.getOrElse(additional)
	/**
	  * Merges an additional condition with the existing global condition
