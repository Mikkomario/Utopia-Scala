...
/**
  * An interface used for targeting multiple rows with each query, grouping the results.
  * @author Mikko Hilpinen
  * @since 20.01.2026, v2.1
  */
trait AccessGroupedLike[+A, +Repr] extends TargetingGroupedLike[A, Repr]
{
	// ABSTRACT ----------------------
	...
	/**
	 * @return An empty item or collection of items (representing 0 rows)
	 */
	...
	/**
	  ...
	  */
	protected def parse(result: ResultStream): A
	...
	override def pull(implicit connection: Connection): A = pullWith[A](toSelect, emptyResult)(parse)
	...
	/**
	  * A generic 'pullWith' implementation that pulls multiple items at once
	  * @param statement Statement to execute - condition, ordering & finalization will be applied, also
	  * @param condition Where condition to apply. Default = current [[appliedCondition]].
	  * @param ordering Ordering to apply. Default = This access point's [[ordering]].
