...
object OneDeprecatingRoot extends DeprecatingRootFactory[OneDeprecatingRoot]
{
	// IMPLEMENTED    ------------------
	
	override protected def _apply[A <: FilterableView[A]](all: A, conditions: Deprecates): OneDeprecatingRoot[A] =
		_OneDeprecatingRoot(all, conditions)
	
	
	// NESTED   ------------------------
	
	private case class _OneDeprecatingRoot[+A <: FilterableView[A]](all: A, model: Deprecates)
		extends OneDeprecatingRoot[A]
}
 ...
 */
trait OneDeprecatingRoot[+A <: FilterableView[A]] extends DeprecatingRoot[A]
{
	// OTHER    -------------------------
