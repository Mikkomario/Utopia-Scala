...
object DescribedInvitation extends FromModelFactory[DetailedInvitation]
{
	override def apply(model: template.Model[Property]) = Invitation(model).map { base =>
		DetailedInvitation(base,
			model("organization_descriptions").getVector.flatMap { _.model }.flatMap { DescriptionLinkOld(_).toOption }.toSet,
...
/**
  ...
  * @since 11.7.2020, v1
  */
case class DescribedInvitation(wrapped: Invitation, organizationDescriptions: Set[DescriptionLinkOld],
