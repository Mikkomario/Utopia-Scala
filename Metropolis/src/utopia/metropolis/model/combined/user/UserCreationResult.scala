...
{
	private val schema = ModelDeclaration("id" -> IntType, "device_id" -> IntType, "session_key" -> StringType,
		"data" -> ModelType)
	...
	override def apply(model: template.Model[Property]) = schema.validate(model).toTry.flatMap { valid =>
		UserWithLinks(valid("data").getModel).map { userData =>
			UserCreationResult(valid("id"), valid("device_id"), userData, valid("session_key"), valid("device_key"))
		}
	}
}
...
/**
  ...
  * @since 3.12.2020, v1
  */
case class UserCreationResult(userId: Int, deviceId: Int, userData: UserWithLinks, sessionKey: String,
