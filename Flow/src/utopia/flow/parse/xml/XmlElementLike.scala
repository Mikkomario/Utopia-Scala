...
trait XmlElementLike[+Repr <: XmlElementLike[Repr]] extends TreeLike2[Repr] with ModelConvertible with View[Value]
{
	...
	/**
	 * @return Equality function used for comparing XML element names (case insensitive)
	 */
	implicit def nameEquals: EqualsFunction[NamespacedString] = NamespacedString.approxEquals
	 ...
	 */
	@deprecated("Deprecated for removal. Please use name instead", "v2.9")
	def nav = name
	 ...
	 */
	def valueOf(key: NamespacedString) = valueOfAttribute(key).orElse { valueOfChild(key.local) }
	/**
