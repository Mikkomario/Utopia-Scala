package utopia.flow.collection.mutable.typeless
...
import utopia.flow.generic.{PropertyGenerator, SimpleConstantGenerator, SimpleVariableGenerator}
...
class Model[Attribute <: Variable](val attributeGenerator: PropertyGenerator[Attribute])
    extends typeless.Model[Attribute]
{
    // ATTRIBUTES    --------------
     ...
     */
    def update(data: typeless.Model[Property]): Unit = data.attributes.foreach(update)
