...
import utopia.flow.async.MapCollectionParallel
...
object CollectionExtensions
{
	...
	implicit class RichIterableOnce[A](val i: IterableOnce[A]) extends AnyVal
	{
		 ...
		 * @return Access to parallel mapping functions
		 */
		def parallel(implicit exc: ExecutionContext) = MapCollectionParallel(i)
		...
		/**
		 * Groups the items in this collection, and them maps each group
		 * @param toKey A function which maps an item to a mapping key
		 * @param toValue A function which maps an item to an individual values
		 * @param mapValues A function which maps the collected values, where each value belongs to the same key
		 * @tparam K Type of keys
		 * @tparam VP Type of individual values
		 * @tparam V Type of mapped values
		 * @return A map where keys are generated by 'toKey' and values are results of 'mapValues'
		 */
		def groupMapMap[K, VP, V](toKey: A => K)(toValue: A => VP)(mapValues: Seq[VP] => V) =
			groupMapUsing(OptimizedIndexedSeq.newBuilder[VP].mapResult(mapValues))(toKey)(toValue)
	...
	class IterableOperations[Repr, I <: IsIterable[Repr]](coll: Repr, iter: I)
	{
		...
		/**
		 * Splits this collection into two parts, based on a find function
		 * @param f A function that returns true for the item on which the split is performed
		 * @param bf Implicit build-from for the resulting collections
		 * @tparam To Type of the resulting collections
		 * @return A pair of collections, where:
		 *              1. The first element contains all items of this collection before the item on which
		 *                 'f' yielded true (exclusive).
		 *              1. The second element contains the item for which 'f' yielded true,
		 *                 and all items which appear after that in this collection.
		 *
		 *         If 'f' yielded false for all items,
		 *         the first collection will contain all elements from this collection
		 *         and the second collection will be empty.
		 */
		def splitAtFirstWhere[To](f: iter.A => Boolean)(implicit bf: BuildFrom[Repr, iter.A, To]) = {
			// Collects the items to this builder while looking for the match
			val leftBuilder = bf.newBuilder(coll)
			val iter = ops.iterator
			
			// Iterates until a match is found
			var splitter: Option[this.iter.A] = None
			while (splitter.isEmpty && iter.hasNext) {
				val next = iter.next()
				// Case: Match found => Remembers it and stops iterating
				if (f(next))
					splitter = Some(next)
				// Case: Not a match => Moves to the next item
				else
					leftBuilder += next
			}
			
			val left = leftBuilder.result()
			splitter match {
				// Case: Match found => Builds the remainder into a separate collection
				case Some(splitter) =>
					if (iter.hasNext)
						Pair(left, bf.fromSpecific(coll)(Iterator.single(splitter) ++ iter))
					else
						Pair(left, bf.fromSpecific(coll)(Single(splitter)))
				
				// Case: No match found => The right side remains empty
				case None => Pair(left, bf.fromSpecific(coll)(Empty))
			}
		}
		...
		def mapHead[B >: iter.A, To](f: iter.A => B)(implicit bf: BuildFrom[Repr, B, To]): To = {
			...
			iter.nextOption() match {
				...
				case None => bf.fromSpecific(coll)(Empty)
			}
		}
		 ...
		 *         where 'f' threw or some other exception occurred.
		  */
		@deprecated("Deprecated for removal. Please use .parallel functions", "v2.8")
		def mapAllParallel[B, To <: Iterable[_]](f: iter.A => B)
		                                        (implicit bf: BuildFrom[Repr, B, To], exc: ExecutionContext) =
			ops.parallel.map(f).to(bf.newBuilder(coll)).all
		/**
		...
		def mapParallel[B, To <: Iterable[_]](maxWidth: Int, bufferSize: => Int = 8)(f: iter.A => B)
			...
			ops.parallel.map(f).to(bf.newBuilder(coll))(maxWidth, bufferSize)
	}
