    ...
    extends ModelConvertible
{
    ...
    /**
      ...
      */
    def isProbablyValid(model: typeless.Model[Property]) =
    {
    ...
    /**
      ...
      */
    def validate(model: typeless.Model[Property]): ModelValidationResult =
    {
                ...
                find(att.name) match {
                    case Some(declaration) =>
                        att.value.castTo(declaration.dataType) match {
                            ...
                            case Some(castValue) => castBuilder += value.typeless.Constant(declaration.name, castValue)
                            // Case: Casting failed
                            case None =>
                                ...
                                ModelValidationResult.success(model,
                                    Model.withConstants(resultConstants ++
                                        childResults.map { case (childName, result) =>
                                            value.typeless.Constant(childName, result.success.get) }))
                        }
                    }
                }
            }
