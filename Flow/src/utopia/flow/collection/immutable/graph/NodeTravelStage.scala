...
/**
  ...
  * May represent either a completed or an ongoing search.
  * @param node Last visited node. If 'isDestination' is true, this is a searched node.
  * @param routes The currently discovered cheapest routes to that node. Never empty.
  ...
  * @param isDestination Whether 'node' represents a search destination.
  *                      False if this represents potential progress in the node-search process instead.
  * @param isConfirmedAsOptimal True if 'routes' has been confirmed to be the optimal /
 *                             the cheapest possible routes to 'node'.
  *                             If false, another route or routes may be discovered,
  *                             that are at least as valid.
  *
  ...
  * @tparam C Type of the counted cost
  */
