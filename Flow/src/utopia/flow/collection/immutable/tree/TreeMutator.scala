...
object TreeMutator
{
	 ...
	 */
	def apply[N, Node <: CopyableTreeLike[Node]](root: Node, path: Seq[N])
 ...
 * @tparam N Type of the accepted navigational elements
	...
	extends TreeNavigator[N, TreeMutator[N, Node]] with CopyableTreeLike[Node]
{
	...
	override val children: Seq[TreeMutator[N, Node]] =
		node.children.iterator.map { c => new TreeMutator(root, path :+ c, c)(matcher)(navToNode) }.caching
	...
	override def self: TreeMutator[N, Node] = this
	
	override protected def matches(node: TreeMutator[N, Node], nav: N): Boolean = matcher(node.node, nav)
	
	override protected def nodeFor(nav: N): TreeMutator[N, Node] = {
		val node = navToNode(nav)
		new TreeMutator[N, Node](root, path :+ node, node, generated = true)(matcher)(navToNode)
	}
	
	override def withChildren(newChildren: IterableOnce[Node]): Node = {
		newChildren.nonEmptyCollection match {
