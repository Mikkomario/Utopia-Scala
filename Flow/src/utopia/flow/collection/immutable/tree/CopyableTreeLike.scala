...
import utopia.flow.collection.template.tree.{Tree, TreeLike2, ValueTreeLike}
...
trait CopyableTreeLike[Repr <: CopyableTreeLike[Repr]] extends TreeLike2[Repr]
{
	 ...
	 */
	def withChildren(newChildren: Seq[Repr]): Repr
	...
	/**
	  ...
	  */
	def :+(tree: Repr) =
		withChildren(children.mergeOrAppend(tree) { _.nav ~== tree.nav } { _ :++ _.children })
	/**
	 ...
	 * @return a copy of this tree with the specified child included / merged in
	 */
	def draftIn(child: Repr)(matches: (Repr, Repr) => Boolean)(merge: (Repr, Repr) => Repr) =
		withChildren(children.mergeOrAppend(child) { matches(_, child) } { (existing, newEntry) =>
			val merged = merge(existing, newEntry)
			if (merged != existing)
				???
			else
				merged
