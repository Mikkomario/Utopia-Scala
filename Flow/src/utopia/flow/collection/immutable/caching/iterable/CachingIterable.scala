...
object CachingIterable extends IterableFactory[CachingIterable]
{
	// ATTRIBUTES   ------------------------
	
	private val _empty = new CachingIterable(Iterator.empty)
	
	
	// IMPLEMENTED  ------------------------
	
	override def from[A](source: IterableOnce[A]) = source match {
		case c: CachingIterable[A] => c
		case c => new CachingIterable[A](c.iterator)
	}
	
	override def empty[A]: CachingIterable[A] = _empty
	
	override def newBuilder[A] =
		new VectorBuilder[A]().mapResult { v => new CachingIterable[A](v.iterator) }
	...
	/**
	  ...
	  */
	def single[A](item: => A) = new CachingIterable[A](PollableOnce(item))
	
	def fromFunctions[A](items: (() => A)*): CachingIterable[A] = apply(items.map { _() })
}
...
/**
  ...
  */
class CachingIterable[+A](source: Iterator[A])
	extends Iterable[A] with IterableOps[A, CachingIterable, CachingIterable[A]]
{
	// ATTRIBUTES   ----------------------------
	...
	override def empty = CachingIterable.empty
	override def iterableFactory = CachingIterable
	...
	override def toSeq = toVector
	
	override def isEmpty = builder.isEmpty && source.isEmpty
	override def size = builder.currentSize + AppendingIterator.size
	...
	override protected def fromSpecific(coll: IterableOnce[A @uncheckedVariance]) =
		CachingIterable.from(coll)
	override protected def newSpecificBuilder: mutable.Builder[A @uncheckedVariance, CachingIterable[A]] =
		CachingIterable.newBuilder
	...
	override def filterNot(pred: A => Boolean) = super.filterNot(pred)
