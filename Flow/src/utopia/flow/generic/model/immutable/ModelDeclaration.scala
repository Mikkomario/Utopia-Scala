    ...
    extends ModelConvertible
{
    ...
    /**
      ...
      */
    class DeclaredPropertyFactory[+P <: Property](actualize: (String, Value) => P) extends PropertyFactory[P]
    {
        override def apply(propertyName: String, value: Value) = {
                ...
                .orElse { declarations.find { _.alternativeNames.exists { _ ~== propertyName } } } match
            {
                ...
                case Some(declaration) =>
                    val actualValue = {
                        // Passes the default value instead of empty values
                        if (value.isEmpty)
                            declaration.defaultValue
                        // Alternatively, casts the proposed value to the right data type
                        else
                            value.castTo(declaration.dataType).getOrElse(declaration.defaultValue)
                    }
