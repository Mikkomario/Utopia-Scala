...
object Variable
{
	...
	private abstract class AbstractVariable(override val name: String, initialValue: Value = Value.empty)
	                                       (implicit override val listenerLogger: Logger)
		extends ChangingWrapper[Value] with Variable
	{
		// ATTRIBUTES   ---------------------
		
		private val p = EventfulPointer(initialValue)
		override lazy val readOnly: Changing[Value] = ChangingWrapper(p)
		...
		override protected def wrapped: Changing[Value] = p
		
		override def value_=(newValue: Value): Unit = p.value = processValue(newValue)
		override def setAndQueueEvent(newValue: Value): IterableOnce[AfterEffect] =
			p.setAndQueueEvent(processValue(newValue))
	}
