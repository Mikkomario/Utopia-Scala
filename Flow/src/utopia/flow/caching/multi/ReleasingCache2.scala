...
object ReleasingCache2
{
	...
	def apply[K, V <: AnyRef](request: K => V)(calculateReferenceLength: (K, V) => Duration)
		...
		new ReleasingCache2[K, V](request)(calculateReferenceLength)
	...
	/**
	  ...
	  * @return A new cache
	  */
	def apply[K, V <: AnyRef](referenceLength: FiniteDuration)(request: K => V)(implicit exc: ExecutionContext) =
		new ReleasingCache2[K, V](request)((_, _) => referenceLength)
}
...
/**
  ...
  */
class ReleasingCache2[K, V <: AnyRef](request: K => V)(calculateReferenceLength: (K, V) => Duration)
                                     (implicit exc: ExecutionContext)
	extends CacheLike[K, V]
{
	// ATTRIBUTES   ------------------------------
	
	private val cache = ExpiringCache2(request)(calculateReferenceLength)
