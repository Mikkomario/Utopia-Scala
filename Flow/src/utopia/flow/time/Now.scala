...
import java.time.{Instant, LocalDate, LocalDateTime, LocalTime, Period}
...
object Now extends ValueConvertible with RichComparable[Instant]
{
	...
	*/
	...
	/**
	  ...
	  */
	def +(duration: Duration) = toInstant + duration
	
	/**
	  * @param period Time period
	  * @return Time after that period of time has passed
	  */
	def +(period: Period) = toInstant + period
	...
	/**
	  * @param period Time period
	  * @return Time before that period of time passed
	  */
	def -(period: Period) = toInstant - period
