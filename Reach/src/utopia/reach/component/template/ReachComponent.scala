...
/**
  ...
  */
trait ReachComponent extends Stackable with PartOfComponentHierarchy
{
	// ABSTRACT	------------------------
	...
	/**
	  ...
	  */
	def parentCanvas = hierarchy.top
	/**
	  * @return Window that contains this component
	  */
	def parentWindow = hierarchy.parentWindow
	
	/**
	  * @return The absolute (on-screen) position of this component
	  */
	def absolutePosition = position + hierarchy.absolutePositionModifier
	/**
	...
	/**
	  ...
	  */
	def isChildOf(another: ReachComponent) = hierarchy.isChildOf(another)
	/**
	  * @param hierarchy A component hierarchy
	  * @return Whether this component is a child (below) of the specified component hierarchy
	  */
	def isChildOf(hierarchy: ComponentHierarchy) = hierarchy.isChildOf(hierarchy)
	...
	/**
	  ...
	  */
	def repaint() = hierarchy.repaint(bounds)
	/**
	  * Paints this component again
	  */
	def repaint(priority: Priority) = hierarchy.repaint(bounds, priority)
	/**
	  ...
	  *                 (Default = Normal).
	  */
	def repaintParent(priority: Priority = Priority.Normal) = hierarchy.repaintBottom(priority)
	/**
	...
	def repaintArea(relativeArea: Bounds, priority: Priority = Priority.Normal) =
		hierarchy.repaint(relativeArea + position, priority)
	/**
	...
	/**
	  ...
	  */
	def paintMovement(movement: => Vector2D) = hierarchy.shiftArea(bounds, movement)
	/**
	...
	def addHierarchyListener[U](listener: Boolean => U) =
		hierarchy.linkedFlag.addContinuousListenerAndSimulateEvent(false) { e => listener(e.newValue) }
	...
	/**
	  ...
	  */
	def createWindow[C <: ReachComponent, R](alignment: Alignment = Alignment.Right, margin: Double = 0.0,
	...
	def debugPrintHierarchy(indentation: Int = 0): Unit = {
		println(s"${ "\t" * indentation }${ getClass.getSimpleName } (${ hierarchy.isLinked })")
