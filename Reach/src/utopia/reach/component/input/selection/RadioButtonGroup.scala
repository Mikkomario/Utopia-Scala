...
trait RadioButtonGroupSettingsLike[+Repr] extends StackSettingsLike[Repr]
{
	...
	override def withCapPointer(p: Changing[StackLength]): Repr = withCapPointer(p)
		...
		with FromContextComponentFactoryFactory[VariableTextContext, ContextualRadioButtonGroupFactory]
{
	...
	override def withContext(hierarchy: ComponentHierarchy, c: VariableTextContext) =
		ContextualRadioButtonGroupFactory(hierarchy, c, settings)
		...
		with ManyFocusableWrapper
{
	...
	private val (_wrapped, buttons) = Stack(parentHierarchy).withContext(context)
		...
		.build(RadioButtonLine) { baseLineF =>
			...
			val lineF = baseLineF
				.mapContext { c => if (settings.axis == Y) c.withTextExpandingToRight else c }
				.withSelectedColorRole(settings.selectedColorRole)
