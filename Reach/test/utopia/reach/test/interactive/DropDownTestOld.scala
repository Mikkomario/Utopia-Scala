...
import utopia.reach.component.interactive.input.selection.{DropDown, DropDownOld}
...
object DropDownTestOld extends App
{
	...
	val shrinkIcon = arrowImage.map { i => SingleColorIcon(i.transformedWith(Matrix2D.quarterRotationClockwise)) }
		.getOrElse(SingleColorIcon.empty)
	val baseDdf = DropDownOld.withExpandAndCollapseIcon(Pair(expandIcon, shrinkIcon))
