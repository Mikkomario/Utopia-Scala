...
CREATE TABLE `statement`(
	...
	`delimiter_id` INT, 
	`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	INDEX lg_s_created_idx (`created`), 
	CONSTRAINT lg_s_de_delimiter_ref_fk FOREIGN KEY lg_s_de_delimiter_ref_idx (delimiter_id) REFERENCES `delimiter`(`id`) ON DELETE SET NULL
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `word_placement`(
	...
	CONSTRAINT lg_wp_s_statement_ref_fk FOREIGN KEY lg_wp_s_statement_ref_idx (statement_id) REFERENCES `statement`(`id`) ON DELETE CASCADE, 
	CONSTRAINT lg_wp_w_word_ref_fk FOREIGN KEY lg_wp_w_word_ref_idx (word_id) REFERENCES `word`(`id`) ON DELETE CASCADE
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
-- url:      This domain as a string, excluding the protocol part. May include a port number.
-- created:  Time when this domain was added to the database
-- is_https: Whether to connect using HTTPS instead of HTTP. Uncertain if both forms have been encountered.
CREATE TABLE `domain`(
	`id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT, 
	`url` VARCHAR(12) NOT NULL, 
	`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	`is_https` BOOLEAN, 
	INDEX lg_do_url_idx (`url`)
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `request_path`(
	...
	`path` VARCHAR(12), 
	`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	INDEX lg_rp_combo_1_idx (domain_id, path), 
	CONSTRAINT lg_rp_do_domain_ref_fk FOREIGN KEY lg_rp_do_domain_ref_idx (domain_id) REFERENCES `domain`(`id`) ON DELETE CASCADE
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `link`(
	...
	`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	CONSTRAINT lg_l_rp_path_ref_fk FOREIGN KEY lg_l_rp_path_ref_idx (path_id) REFERENCES `request_path`(`id`) ON DELETE CASCADE
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
...
CREATE TABLE `link_placement`(
	...
	`order_index` TINYINT NOT NULL DEFAULT 0, 
	CONSTRAINT lg_lp_s_statement_ref_fk FOREIGN KEY lg_lp_s_statement_ref_idx (statement_id) REFERENCES `statement`(`id`) ON DELETE CASCADE, 
	CONSTRAINT lg_lp_l_link_ref_fk FOREIGN KEY lg_lp_l_link_ref_idx (link_id) REFERENCES `link`(`id`) ON DELETE CASCADE
)Engine=InnoDB DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
