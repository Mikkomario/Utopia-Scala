...
There are a total number of 2 packages and 10 classes

### Text
This package contains the following 6 classes: [Delimiter](#delimiter), [Statement](#statement), [Statement Placement](#statement-placement), [Text Placement](#text-placement), [Word](#word), [Word Placement](#word-placement)
...
- [Statement Placement](#statement-placement).`statementId`
...
##### Properties
Domain contains the following 3 properties:
- **Url** - `url: String` - This domain as a string, excluding the protocol part. May include a port number.
- **Created** - `created: Instant` - Time when this domain was added to the database
- **Is Https** - `isHttps: UncertainBoolean` - Whether to connect using HTTPS instead of HTTP. Uncertain if both forms have been encountered.
