...
case class Angle private(radians: Double)
	extends LinearSizeAdjustable[Angle] with Combinable[DirectionalRotation, Angle] with SelfComparable[Angle]
		with ValueConvertible with ApproxSelfEquals[Angle] with MayBeAboutZero[Angle, Angle]
{
	// ATTRIBUTES    ------------------
	...
	override def zero: Angle = Angle.zero
	override def isZero: Boolean = radians == 0.0
