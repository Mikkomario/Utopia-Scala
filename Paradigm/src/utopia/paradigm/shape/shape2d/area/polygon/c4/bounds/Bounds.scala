...
object Bounds extends BoundsFactoryLike[Double, Point, Size, Bounds] with FromModelFactory[Bounds]
{
	...
	/**
	 * @return A builder that yields the bounds around the specified bounds instances
	 */
	def aroundBuilder = new BoundsAroundBuilder
	...
	class BoundsAroundBuilder extends mutable.Builder[Bounds, Bounds]
	{
		// ATTRIBUTES   -----------------
		
		private var ends: Option[Pair[Point]] = None
		
		
		// COMPUTED ---------------------
		
		/**
		 * @return An interface for building bounds around individual points
		 */
		def points = AroundPointsBuilder
		
		
		// IMPLEMENTED  -----------------
		
		override def clear(): Unit = ends = None
		override def result(): Bounds = ends match {
			case Some(ends) => between(ends)
			case None => zero
		}
		
		override def addOne(elem: Bounds): BoundsAroundBuilder.this.type = {
			ends match {
				case Some(ends) =>
					this.ends = Some(Pair(
						ends.first.topLeft(elem.topLeftCorner), ends.second.bottomRight(elem.bottomRightCorner)))
				case None => ends = Some(Pair(elem.topLeftCorner, elem.bottomRightCorner))
			}
			this
		}
		...
		/**
		 * @return The built bounds. None if no elements were introduced.
		 */
		def resultOption() = ends.map(between)
		
		
		// NESTED   ----------------------
		
		object AroundPointsBuilder extends mutable.Builder[Point, Bounds]
		{
			override def clear(): Unit = BoundsAroundBuilder.this.clear()
			override def result(): Bounds = BoundsAroundBuilder.this.result()
			
			override def addOne(elem: Point): AroundPointsBuilder.this.type = {
				ends match {
					case Some(e) => ends = Some(Pair(e.first.topLeft(elem), e.second.bottomRight(elem)))
					case None => ends = Some(Pair.twice(elem))
				}
				this
			}
		}
