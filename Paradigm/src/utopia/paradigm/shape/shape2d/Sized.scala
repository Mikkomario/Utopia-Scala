...
trait Sized[+Repr]
{
	...
	/**
	  ...
	  */
	def croppedToFit(maxArea: Vector2DLike[_ <: V]) = withSize(size.combineWith(maxArea) { _ min _ })
	/**
	  ...
	  */
	def croppedToFit(maxLength: Vector1D) = withSize(size.mapAxis(maxLength.axis) { _ min maxLength.length })
	/**
	  ...
	  */
	def croppedHorizontallyWithin(maxWidth: Double) = croppedToFit(X(maxWidth))
	/**
	  ...
	  *         If this item already fits within the specified limit, returns this area.
	  */
	def croppedVerticallyWithin(maxHeight: Double) = croppedToFit(Y(maxHeight))
