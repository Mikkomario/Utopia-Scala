...
object YesNoDialog
{
			  ...
			  defaultResult: Boolean = false) =
	{
		...
		new YesNoDialog(textContext, title, question, icons, colors, defaultResult)({ (color, _) =>
...
/**
  ...
  */
class YesNoDialog(override val standardContext: TextContextLike, override val title: LocalizedString,
				 ...
				 (getButtonContext: (ButtonColor, Boolean) => ButtonContextLike) extends InteractionDialog[Boolean]
{
