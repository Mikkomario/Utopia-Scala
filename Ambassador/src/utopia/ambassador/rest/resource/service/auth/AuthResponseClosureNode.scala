	...
	extends LeafResource[AuthorizedContext]
{
	...
	private def closeAuthCase(settings: AuthServiceSettings, authCase: IncompleteAuth, session: Token, userId: Int)
	                         (implicit context: AuthorizedContext, connection: Connection) =
	{
		// Swaps the authentication code for access token(s). Blocks.
		tokenAcquirer.forCode(userId, authCase.code, settings).waitForResult() match
		{
