...
import utopia.ambassador.model.enumeration.AmbassadorScope.OAuth
...
class AuthPreparationNode(target: ServiceTarget) extends LeafResource[AuthorizedContext]
{
	...
	{
		context.authorizedForScope(OAuth) { (session, connection) =>
			implicit val c: Connection = connection
			// The session must be tied to a specific user
				...
				case Some(userId) =>
					...
					context.handlePost(NewAuthPreparation) { preparation =>
						...
						target.settings match {
							case Some(settings) => prepareOAuth(session, settings, userId, preparation)
	...
	private def prepareOAuth(session: Token, settings: AuthServiceSettings, userId: Int,
	                         preparation: NewAuthPreparation)
	                        (implicit connection: Connection, context: AuthorizedContext) =
	{
		...
		if (preparation.coversAllCompletionCases || settings.defaultCompletionRedirectUrl.isDefined)
		{
			...
			if (state.forall { _.length <= maxStateLength })
			{
				...
				val existingScopeIds = userAccess.accessibleScopeIds
				...
				val insertedPreparation = AuthPreparationModel.insert(
					AuthPreparationData(userId, uuidGenerator.next(), Now + settings.preparationTokenDuration, state))
