...
final class ByteBufferParser[J](src: ByteBuffer) extends SyncParser[J] with ByteBasedParser[J] {
  ...
  protected[this] def close(): Unit = (src: Buffer).position(src.limit)
  ...
  protected[this] def byte(i: Int): Byte = src.get(i + start)
  protected[this] def at(i: Int): Char = src.get(i + start).toChar
  ...
  protected[this] def atEof(i: Int): Boolean = i >= limit
}
