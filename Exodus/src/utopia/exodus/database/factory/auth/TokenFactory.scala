	...
	override def apply(model: Model[Property]) = {
		table.validate(model).map{ valid => 
			...
			Token(valid("id").getInt, TokenData(valid("typeId").getInt, valid("hash").getString, 
				valid("parentTokenId").int, valid("ownerId").int, modelStylePreference, valid("expires").instant,
				valid("created").getInstant, valid("deprecatedAfter").instant, valid("isSingleUseOnly").getBoolean))
		}
	}
}
