...
import utopia.exodus.database.model.auth.{EmailValidatedSessionModel, EmailValidationAttemptModel}
			...
			val attempt = find(model.withToken(token).withPurposeId(purposeId).toCondition).map { Right(_) }
				.orElse {
					DbEmailValidatedSession.withToken(token).pull.flatMap { session =>
						...
						DbEmailValidationAttempt(session.validationId).pull.map { attempt => Left(attempt -> session) }
					}
				}
			attempt match
			{
