...
import utopia.exodus.database.access.many.auth.{DbTokenScopeLinks, DbTokenScopes}
...
case class DbSingleToken(id: Int) extends UniqueTokenAccess with SingleIntIdModelAccess[Token]
{
	...
	/**
	  * @return An access point to this token's scopes
	  */
	def scopes = DbTokenScopes.withTokenId(id)
}
