...
import utopia.annex.model.request.{ApiRequest2, ApiRequestSeed2, RequestQueueable2, Retractable}
import utopia.annex.model.response.RequestNotSent2.{RequestSendingFailed2, RequestWasDeprecated2}
import utopia.annex.model.response.{RequestFailure2, RequestResult2, Response}
...
/**
  ...
  */
class QueueSystem2(api: ApiClient, offlineModeWaitThreshold: FiniteDuration = 30.seconds,
                  ...
                  (implicit exc: ExecutionContext)
{
	...
	/**
	  ...
	  */
	def push[A](request: ApiRequest2[A]): Future[RequestResult2[A]] = {
	...
	/**
	  ...
	  * @return Asynchronous final send result
	  */
	def push[A](requestSeed: ApiRequestSeed2[A]): Future[RequestResult2[A]] = push(Left(requestSeed))
	/**
	  * Pushes a new request to this queue system.
	  * The request may be executed immediately, or delayed in case this system is currently in offline mode.
	  * The request is attempted until a non-503-response is received,
	  ...
	  * @return Asynchronous final send result
	  */
	def push[A](request: RequestQueueable2[A]): Future[RequestResult2[A]] = {
			...
			offlineQueue.push { sendBlocking(request, offlineMode = true) }.flatMap {
				...
				case Left(delayedRequestFuture) =>
					delayedRequestFuture.flatMap {
						...
						case Failure(error) => Future.successful(RequestSendingFailed2(error))
					}
	...
	/**
	  ...
	  */
	def pushBlocking[A](request: ApiRequest2[A]): RequestResult2[A] = {
		...
		// Case: Offline => Queues the request
			...
			offlineQueue.push { sendBlocking(request, offlineMode = true, skipWait = false) }
				.waitFor().getOrMap(RequestSendingFailed2.apply)
	}
	/**
	  * Pushes a new request to this queue system.
	  * The request may be executed immediately, or delayed in case this system is currently in offline mode.
	  ...
	  * @return Final send result, once acquired
	  */
	def pushBlocking[A](requestSeed: ApiRequestSeed2[A]): RequestResult2[A] = pushBlocking(Left(requestSeed))
	/**
	  * Pushes a new request to this queue system.
	  * The request may be executed immediately, or delayed in case this system is currently in offline mode.
	  * The request is attempted until a non-503-response is received,
	  ...
	  * @return Final send result, once acquired
	  */
	def pushBlocking[A](request: RequestQueueable2[A]): RequestResult2[A] = {
	...
	def pushSynchronous[A](request: ApiRequest2[A]): RequestResult2[A] = pushBlocking(request)
	
	private def handleDelayedRequestBlocking[A](requestFuture: Future[Try[ApiRequest2[A]]]) =
	{
		requestFuture.waitForResult()
			...
			.getOrMap(RequestSendingFailed2.apply)
	}
	...
	private def sendBlocking[A](request: RequestQueueable2[A],
	                            offlineMode: Boolean = false): Either[Future[Try[ApiRequest2[A]]], RequestResult2[A]] =
	{
		afterPossibleWait[Either[Future[Try[ApiRequest2[A]]], RequestResult2[A]]](request.either,
	...
	private def sendBlocking[A](request: ApiRequest2[A], offlineMode: Boolean, skipWait: Boolean): RequestResult2[A] =
	{
		afterPossibleWait[RequestResult2[A]](request, waitEnabled = offlineMode && !skipWait)(Identity) {
			...
			val result = resultFuture.waitFor(offlineModeWaitThreshold) match {
				...
				case Failure(_) =>
					...
					resultFuture.waitFor() match {
						...
						case Failure(error) => Left(RequestSendingFailed2(error))
					}
			}
			result match {
	...
	private def afterPossibleWait[A](request: Retractable, waitEnabled: Boolean)(fail: RequestFailure2 => A)(f: => A) = {
		...
		if (request.deprecated)
			fail(RequestWasDeprecated2)
		else if (waitInterrupted)
			fail(RequestSendingFailed2(new InterruptedException("Request-sending was interrupted during offline wait")))
		else
			f
	}
}
