...
import utopia.annex.model.request.{Persisting, RequestQueueable2}
...
object PersistingRequestQueue2
{
	// OTHER	-------------------------
	...
	/**
	  ...
	  */
	def apply(master: QueueSystem2, fileLocation: Path, handlers: Iterable[PersistedRequestHandler2],
	          width: Int = 1, saveLogic: SaveTiming = OnJvmClose)
	         (implicit exc: ExecutionContext, jsonParser: JsonParser, logger: Logger): PersistingRequestQueue2 =
	{
	...
	private class SimpleQueue(fileLocation: Path, override val master: QueueSystem2, width: Int = 1,
	                          saveLogic: SaveTiming = OnJvmClose)
	                         (implicit val exc: ExecutionContext, jsonParser: JsonParser, logger: Logger)
		extends PersistingRequestQueue2
	{
		// ATTRIBUTES	----------------
...
/**
  ...
  * @since 17.6.2020, v1
  */
trait PersistingRequestQueue2 extends SystemRequestQueue
{
	// ABSTRACT	-------------------------
	...
	/**
	  ...
	  */
	def withoutPersisting: RequestQueue2 = SkipPersistenceQueue
	
	
	// IMPLEMENTED	---------------------
	
	override def push[A](request: RequestQueueable2[A]) = {
		request match {
	...
	/**
	  ...
	  */
	def processPersistedRequestsUsing(handlers: Iterable[PersistedRequestHandler2]) = {
	...
	def start(handlers: Iterable[PersistedRequestHandler2]) = processPersistedRequestsUsing(handlers)
	
	private def pushPersisting[A](request: Persisting)(pushRequest: => QueuedAction[RequestResult2[A]]) =
	{
	...
	private object SkipPersistenceQueue extends RequestQueue2
	{
		override def push[A](request: RequestQueueable2[A]): QueuedAction[RequestResult2[A]] =
			PersistingRequestQueue2.super.push(request)
	}
}
