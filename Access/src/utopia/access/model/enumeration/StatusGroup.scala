...
object StatusGroup
{
	...
	/**
	  * All status group values, including Custom.
	  */
	lazy val values = Vector[StatusGroup](Information, Success, Redirect, ClientError, ServerError, Custom)
	/**
	  * Neutral status groups which may or may not be interpreted as success
	  */
	lazy val neutral = Pair[StatusGroup](Information, Redirect)
	/**
	  * Failure status groups which represent an error in either server or client side
	  */
	lazy val failure = Pair[StatusGroup](ClientError, ServerError)
