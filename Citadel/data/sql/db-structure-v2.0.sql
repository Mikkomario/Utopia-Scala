...
-- role_id: Id of the organization member role that has authorization to perform the referenced task
-- task_id: Id of the task the user's with referenced membership role are allowed to perform
...
CREATE TABLE `user_role_right`(
	...
	`task_id` INT NOT NULL, 
	`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	CONSTRAINT urr_mr_role_ref_fk FOREIGN KEY urr_mr_role_ref_idx (role_id) REFERENCES `member_role`(id) ON DELETE CASCADE, 
...
-- deprecated_after: Time when this MemberRole became deprecated. None while this MemberRole is still valid.
...
CREATE TABLE `language_familiarity_description`(
	...
	`description_id` INT NOT NULL, 
	CONSTRAINT lfd_lf_language_familiarity_ref_fk FOREIGN KEY lfd_lf_language_familiarity_ref_idx (familiarity_id) REFERENCES `language_familiarity`(id) ON DELETE CASCADE, 
...
-- created: Time when this UserLanguage was first created
