	...
	extends ManyModelAccess[A] with Indexed with FilterableView[Repr]
{
	...
	/**
	  ...
	  * @return Whether any Membership instance was affected
	  */
	def endTimes_=(newEnded: Instant)(implicit connection: Connection) = putColumn(model.endedColumn,newEnded)
	...
	/**
	  ...
	  */
	def inOrganizationWithId(organizationId: Int) = filter(model
