...
object IssueVariantData extends FromModelFactoryWithSchema[IssueVariantData]
{
	...
	override lazy val schema = ModelDeclaration(Vector(
		PropertyDeclaration("issueId", IntType, Single("issue_id")),
		PropertyDeclaration("version", AnyType),
		PropertyDeclaration("errorId", IntType, Single("error_id"), isOptional = true),
...
/**
  ...
  */
case class IssueVariantData(issueId: Int, version: Version, errorId: Option[Int] = None, details: Model = Model.empty,
                            created: Instant = Now)
	extends IssueVariantFactory[IssueVariantData] with ModelConvertible
{
	// IMPLEMENTED	--------------------
	
	override def toModel = Model(Vector(
		"issueId" -> issueId, "version" -> version.toString, "errorId" -> errorId, "details" -> details,
		"created" -> created))
